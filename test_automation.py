...
ROOT_DIR = SCRIPT_DIR if (SCRIPT_DIR / ".git").exists() or (SCRIPT_DIR / "README.md").exists() else SCRIPT_DIR.parent
TESTS_DIR = ROOT_DIR / "test_automation"

DEFAULT_EXCEL_CANDIDATES = [
    str(ROOT_DIR / "Assignment 1 - Test cases.xlsx"),
    str(ROOT_DIR / "test_cases.xlsx"),
    str(TESTS_DIR / "Assignment 1 - Test cases.xlsx"),
    str(TESTS_DIR / "test_cases.xlsx"),
]
...
def _wait_for_button_ready(button_locator, timeout_ms: int):
...
def _trigger_action(page, action_locator, timeout_ms: int):
    if not action_locator:
        return
    _wait_for_button_ready(action_locator, timeout_ms)
    try:
        action_locator.scroll_into_view_if_needed(timeout=2000)
    except Exception:
        pass
    try:
        action_locator.click(timeout=max(1000, int(timeout_ms)))
    except Exception:
        action_locator.click(timeout=max(1000, int(timeout_ms)), force=True)

def _wait_for_fresh_output(page, is_chat: bool, output_locator, previous_output: str, retries: int, retry_wait_ms: int) -> str:
    actual_output = ""
    stable_reads = 0
    last_seen = None
    ...
    for _ in range(tries):
        ...
        if not current:
            stable_reads = 0
            page.wait_for_timeout(max(0, int(retry_wait_ms)))
            continue
        if previous_output and current == previous_output:
            stable_reads = 0
            page.wait_for_timeout(max(0, int(retry_wait_ms)))
            continue
...
def _load_frontend(page, args):
    ...
    if is_chat:
        ...
        action_locator = None
    return is_chat, input_locator, output_locator, action_locator

def _fallback_output_path(path: str) -> str:
    source = Path(path)
    return str(source.with_name(f"{source.stem} - updated{source.suffix}"))

def _save_workbook(wb, output_path: str) -> tuple[bool, str]:
    try:
        wb.save(output_path)
        ...
        wb.save(fallback)
        print(f"Output file was locked. Saved results to fallback file: {fallback}")
...
def run_test():
    ...
    with sync_playwright() as p:
        ...
        # 3. Open Frontend
        try:
            is_chat, input_locator, output_locator, action_locator = _load_frontend(page, args)
        ...
        # 4. Iterate Rows
        processed = 0
        start_row = max(header_row + 1, int(args.start_row)) if int(args.start_row or 0) > 0 else header_row + 1
        end_row = min(int(ws.max_row or 0), int(args.end_row)) if int(args.end_row or 0) > 0 else int(ws.max_row or 0)
        ...
        for row_index in range(start_row, end_row + 1):
            ...
            try:
                ...
                _trigger_action(page, action_locator, int(args.timeout_ms))

                page.wait_for_timeout(max(0, int(args.wait_ms)))

                actual_output = _wait_for_fresh_output(
                    page,
                    is_chat,
                    output_locator,
                    prev_output,
                    int(args.retries),
                    int(args.retry_wait_ms),
                )
                ...
                if args.save_every and int(args.save_every) > 0 and processed % int(args.save_every) == 0:
                    _, args.output = _save_workbook(wb, args.output)
                ...
                try:
                    is_chat, input_locator, output_locator, action_locator = _load_frontend(page, args)
                    print("Recovered frontend after error.")
                except Exception as reload_error:
                    print(f"Frontend recovery failed: {reload_error}")
